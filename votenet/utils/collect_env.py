...
def collect_env_info():
    ...
    # theory votenet should be made runnable with only the corresponding runtimes
    ...
    try:
        ...
        data.append(
            ("votenet", votenet.__version__ + " @" + os.path.dirname(votenet.__file__))
        )
    except ImportError:
        data.append(("votenet", "failed to import"))

    try:
        from votenet import _C
    except ImportError:
        data.append(("votenet._C", "failed to import. votenet is not built correctly"))
        ...
        if has_cuda:
            data.append(
                ("votenet arch flags", detect_compute_compatibility(CUDA_HOME, _C.__file__))
            )
