...
@ROI_BOX_HEAD_REGISTRY.register()
class StandardBoxHead2(nn.Module):
    """
    A head with several 3x3 conv layers (each followed by norm & relu) and then
    several fc layers (each followed by relu).
    """
    ...
    def __init__(
        ...
        for layer in cls_convs + reg_convs:
            if isinstance(layer, nn.Conv1d):
                weight_init.c2_msra_fill(layer)
    ...
    def forward(self, x):
        ...
        cls_features = self.cls_convs(x)
        reg_features = self.reg_convs(x)

        pred_cls_logits = self.cls_predictor(cls_features).permute(0, 2, 1)
        pred_box_deltas = self.box_predictor(reg_features).permute(0, 2, 1)
        if not self.cls_agnostic_bbox_reg:
            pred_box_deltas = pred_box_deltas.view(
                batch_size, num_proposals, self.num_classes, 6
            )  # (bs, num_proposals, num_classes, 6)
        ...
        if not self.use_axis_aligned_box:
            # (bs, num_proposals, num_classes)
            pred_heading_deltas = self.box_angle_predictor(reg_features).permute(0, 2, 1)

        pred_centerness = None
        if self.use_centerness:
            # (bs, num_proposals)
            pred_centerness = self.centerness_predictor(reg_features).view(batch_size, num_proposals)
