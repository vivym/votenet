...
@RPN_HEAD_REGISTRY.register()
class StandardRPNHead2(nn.Module):
    """
    Standard RPN classification and regression heads described in :paper:`Faster R-CNN`.
    Uses a 3x3 conv to produce a shared hidden state from which one 1x1 conv predicts
    objectness logits for each anchor and a second 1x1 conv predicts bounding-box deltas
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor):
        ...
        cls_features = self.cls_convs(features)
        reg_features = self.reg_convs(features)

        # (bs, num_proposals, 2)
        pred_objectness_logits = self.objectness_predictor(cls_features).permute(0, 2, 1)
        # (bs, num_proposals, 6)
        pred_box_reg = self.box_predictor(reg_features).permute(0, 2, 1)
        if self.use_exp:
            pred_box_reg = pred_box_reg.exp()
        ...
        if self.use_centerness:
            # (bs, num_proposals)
            pred_centerness = self.centerness_predictor(reg_features).squeeze(1)
